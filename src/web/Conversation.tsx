...
export function Conversation({ session, project, interactions, cursor, canAct, onRefresh, onInteraction, onBack }: {
  ...
  return <article className="conversation">
    <header className="conversation-heading"><div className="conversation-title"><Button className="icon-button subtle mobile-back" onClick={onBack} aria-label="Oturum listesine dön"><ArrowLeft size={22} /></Button><div className="conversation-project-icon"><Terminal size={21} /></div><div><span className="project-name">{project.name}</span><h2>{session.title}</h2></div><StatusBadge state={session.state} /></div><div className="target-summary"><span>{project.host}</span><code>{project.cwd}</code></div><div className="session-identity"><code title={session.id}>Oturum: {session.id.slice(0, 8)}</code><code title={session.generation}>Nesil: {session.generation.slice(0, 8)}</code></div><div className="conversation-toolbar"><span className="source-label">{session.source === 'managed' ? 'intRem ile başlatıldı' : 'Mevcut oturumdan devralındı'}</span><Button className="subtle compact" onClick={() => setDetails(!details)} aria-expanded={details}><FileText size={15} />Oturum bilgisi<ChevronDown size={15} /></Button></div>
      {details && <div className="session-details"><dl><div><dt>İstenen model</dt><dd>{session.requestedModel || 'Bilinmiyor'}</dd></div><div><dt>Bildirilen son model</dt><dd>{session.actualModel || 'Bilinmiyor'}<p className="hint">Model kimliği Claude yanıtından gelir. OmniRoute’un kullandığı model ve hesap ayrıca doğrulanmadı.</p></dd></div><div><dt>Hesap</dt><dd>{session.account || 'Bilinmiyor'}</dd></div><div><dt>Son ilerleme</dt><dd>{displayTime(session.lastActivityAt)}</dd></div><div><dt>Kontrol</dt><dd>{session.controlEnabled ? 'Mesaj gönderebilir · İzin yanıtlayabilir' : 'Yalnız izleme'}</dd></div><div><dt>Kaynak Claude oturumu</dt><dd className="mono">{session.claudeSessionId || 'Henüz başlamadı'}</dd></div>

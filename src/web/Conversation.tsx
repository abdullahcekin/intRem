...
export function Conversation({ session, project, interactions, cursor, canAct, onRefresh, onInteraction, onBack, focusMode, onToggleFocus, listHidden, onToggleList }: {
  ...
  return <article className={`conversation ${focusMode ? 'is-focused' : ''} ${focusMode && !focusWriting ? 'reading-only' : ''}`} aria-label={`${project.name} konuşması`}>
    <header className="conversation-heading"><div className="conversation-title">
      ...
      {!session.controlEnabled && <Notice tone="info">Bu bağlantı kaynak geçmişini izler; terminalin canlı kontrolünü devralmaz. Kayda geçen sorular burada okunabilir, yanıt ve izinler kaynak terminalden verilir. Bu sorular için intRem onay bildirimi göndermez. Kaynak süreç çıktıktan sonra kontrollü devir ile aynı konuşmayı intRem’de sürdürebilirsiniz.</Notice>}
    ...
    <div className="composer-area" id="conversation-composer">
      {!session.controlEnabled && <div className="conversation-notice"><span><LockKeyhole size={14} />Yalnız izleme</span><Button className="subtle compact" onClick={() => setConfirm('takeover')} disabled={!canAct || busy}><ArrowUpRight size={16} />Kontrollü devral</Button></div>}

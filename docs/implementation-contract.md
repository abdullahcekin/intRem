...
## Kaynak oturumun canlı soru bağlantısı

- Kullanıcının **Canlı sorular** kurulumu, izinli proje dizinindeki yerel Claude ayarına `PreToolUse` / `AskUserQuestion` komut hook'u ekler. Mevcut JSON ve hook'lar korunur; özel yedek alınır. Sembolik bağlantı, bozuk JSON ve kurulum sırasında değişen dosya reddedilir. Oturumun `sourceQuestions` ayarı ayrı tutulur; aynı projedeki diğer kaynaklar kendiliğinden yetki kazanmaz.
- Linux API'si aynı kullanıcıya ait `0600` Unix socket dinler. Güven sınırı işletim sistemi kullanıcısıdır; aynı kullanıcı hesabını paylaşan süreçler birbirinden yalıtılmış sayılmaz. Kaynak UUID/PID/başlangıç zamanı, izinli gerçek cwd, hook sürecinin sahipliği ve kaynak sürecin altından geldiği doğrulanır. Alt ajan soruları kapsam dışıdır.
- Yalnız bağlı canlı hook `origin=source_hook` etkileşimi oluşturur. `controlEnabled=false` kalır; normal mesaj/araç izni yetkisi açılmaz. Karar API'si cihaz oturumu, Origin, CSRF, güncel kaynak bağlantısı, soru içeriği ve nesli doğrular. Oturum ayarı kapatılırken ilk bağlantı doğrulaması sürse bile soru yetki kazanamaz.
- İlk geçerli insan kararı kazanır. Yanıt, Claude'un `updatedInput.answers` alanında aynı soruya gönderilir. Kaynak veya hook kimliği değişirse, API kapanırsa, bağlantı kesilirse veya 13 dakika dolarsa uygulanmamış soru iptal/expire olur. Aynı araç çağrısı yeniden bağlanarak eski cevabı alamaz.
- `appliedAt`, kaynak hook için bir kez gönderim hakkının tüketildiğini kaydeder; Claude'un cevabı işlediğine dair ACK değildir. Socket yazımı belirsizse cevap yeniden gönderilmez. Kaynak geçmişindeki başarılı araç sonucu ayrı kanıttır; arayüz kaydedilen kararla kaynakta görülen sonucu ayırır.
- Kaynak ayarının yüklenmesi ve yeni hook çağrısının gelmesi gerekir. Kurulum, geçmişte açılmış bir terminal sorusunu canlı karta dönüştürmez. Hook erişilemezse kaynak kendi soru akışına döner. Terminale tuş basılmaz, kaynak süreç kapatılmaz, ücretli sağlayıcıya geçilmez. Fiziksel telefon push kabulü ayrı doğrulanır.

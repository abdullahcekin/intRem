...
- Kaynak sürümü **`3e4bfedf4ad2bab2926eeafdfbdbc9842f0c3a5d`**, `main` dalına yayınlandı ve normal API servisine dağıtıldı; runner ve kaynak tmux süreçleri korundu. Sonraki yalnız belge commit'leri için canlı Git/CI durumunu ayrıca kontrol edin.
- Bu kaynak sürümü için [main GitHub Actions başarılı](https://github.com/abdullahcekin/intRem/actions/runs/34809441729): Node.js 22, tip kontrolü, **140/140 test**, üretim derlemesi ve **21 Chromium kontrolü**. İki gerçek Linux alt süreç testi bu sayıya dahildir.
- Sistem ekranına OmniRoute kurulum görünürlüğü eklendi: kayıtlı hesap bağlantısı ve model havuzu toplamları, ayrı erişim durumları ve son kontrol zamanı. Başarısız okuma `null/bilinmiyor`, doğrulanmış boş liste `0` olur. Sunucu token'ı yalnız sabit iki GET yoluna gönderilir; hesap ayrıntıları/ham yanıtlar taşınmaz. Süre/boyut sınırı, yönlendirme yasağı, 30 saniye önbellek ve public/private sağlık ayrımı test edildi.
- Kurulu OmniRoute 3.8.50 kaynaklarında GET/auth sözleşmesi doğrulandı. Canlı derlemenin mevcut sunucu ortamıyla salt okunur çağrısında her iki sayaç **`auth_required`**, sayılar `null`; gerçek hesap envanteri henüz okunamıyor. `read` kapsamlı CLI erişim token'ı kurulumu ayrı adımdır. Token veya hesap oluşturulmadı, model isteği gönderilmedi. Sayımlar gerçek istekte kullanılan hesap/model veya bütçe garantisi değildir.
- Yeni backend regresyonları ve mobil senaryo önce başarısız, uygulama sonrası başarılı oldu; bağımsız diff incelemesinde engelleyici bulgu yok. 320/375 px'de yetki/bilinmiyor ve 0/pozitif durumları test edildi, screenshotlar incelendi. Canlı `/info` içindeki OmniRoute açıklaması açıldı; HTTPS sağlık/rehber 200, girişsiz ayrıntılı sağlık/snapshot 401.
...
- Uygulama commit'i: **`62e140b7fcdced1da0a70c91c1309cc937f929b6`**; [yayın CI kanıtı](https://github.com/abdullahcekin/intRem/actions/runs/34795654910). Aktif mesaj/karar/inceleme sayıları sıfır doğrulandı; API durduktan sonra tekrar denetlendi ve runner kontrollü kapandı. Özel veri ve eski build yedeği alındı, SQLite bütünlüğü doğrulandı; temiz checkout fast-forward edildi ve build geçti. İki servis güncel sürümle active, heartbeat güncel; iç/dış sağlık 200, rehber 200, girişsiz API 401. Kaynak tmux pane PID'leri değişmedi. Şema değişikliği yok.
...
3. **[#5: OmniRoute yönlendirmesi](https://github.com/abdullahcekin/intRem/issues/5).** Sağlık, salt okunur kurulum sayımı/erişim görünümü ve SDK üzerinden güvenli hata/kota açıklamaları uygulandı. Kurulum sayımı için sunucu `read` token'ı bekleniyor; canlı API okuması yetki istiyor. Üç hesap zinciri, ayrı model havuzları, güvenilir hesap telemetrisi ve bütçeli fallback henüz tamamlanmadı. 2026-09-13 veritabanı sayımında hesap/combo/eşleme sayıları sıfırdı; bu sayım güncel hesap envanteri yerine geçmez. Kurulu 3.8.50 bütçe kontrolü atomik harcama rezervasyonu yapmıyor ve sıfır limit sınırsız anlamına geliyor; sert bütçe garantisi olarak kullanmayın, ücretli fallback kapalı kalmalı.
...
Son hata açıklaması tesliminin kaynak/diff incelemesi ana ajan tarafından yapıldı. Paralel inceleme ajanları kredi engelinde durduğu için bu değişiklik için bağımsız final review kanıtı yoktur. Kod, sentetik SDK olayları ve kalıcı Store üzerinden sınandı; yayın CI'sı ve normal servis dağıtımı yukarıda kayıtlıdır.
...
Sonraki somut adım: gateway yöneticisi `read` kapsamlı CLI erişim token'ını özel sunucu ortamına ekledikten sonra Sistem ekranındaki hesap/havuz sayılarını gerçek kayıtlarla doğrulamak. Token'ı sohbet veya Git üzerinden taşımayın; [kurulum rehberindeki](deployment.md#omniroute-kurulum-görünürlüğü) sınırları koruyun. Sıradaki geliştirme adayı: #5 kapsamında model eşlemeleri ve ayrı havuz kurulumunun salt okunur doğrulanabilir sözleşmesini incelemek; mevcut toplamları havuz doğruluğu kabulü saymamak. Kabul işleri: ilk fiziksel cihaz/PWA/push ve kapasite sağlandığında normal runner plan pilotu ile gerçek sağlayıcı hata eşliği. Hesap/bütçe kurulumu ayrı kalır. Yerel `tasks/` altındaki test raporları ve özel pilot betikleri Git'e eklenmez; önceki tamamlanmış pilotları otomatik tekrarlamayın.

...
- Kaynak sürümü **`62e140b7fcdced1da0a70c91c1309cc937f929b6`**, `main` dalına yayınlandı ve normal API + runner'a dağıtıldı. Sonraki yalnız belge commit'leri için canlı Git/CI durumunu ayrıca kontrol edin.
- Bu kaynak sürümü için [main GitHub Actions başarılı](https://github.com/abdullahcekin/intRem/actions/runs/34795654910): Node.js 22, tip kontrolü, **92/92 test**, üretim derlemesi ve **20 Chromium kontrolü**. İki gerçek Linux alt süreç testi bu sayıya dahildir.
...
3. **[#5: OmniRoute yönlendirmesi](https://github.com/abdullahcekin/intRem/issues/5).** Sağlık görünümü ve SDK üzerinden güvenli hata/kota açıklamaları uygulandı. Üç hesap zinciri, ayrı model havuzları, güvenilir hesap telemetrisi ve bütçeli fallback henüz tamamlanmadı. 2026-09-13 yeniden sayımında hesap/combo/eşleme sayıları sıfırdı; bu sayım güncel hesap envanteri yerine geçmez. Kurulu 3.8.50 bütçe kontrolü atomik harcama rezervasyonu yapmıyor ve sıfır limit sınırsız anlamına geliyor; sert bütçe garantisi olarak kullanmayın, ücretli fallback kapalı kalmalı.
...
Sıradaki geliştirme adayı: OmniRoute'un mevcut API yeteneklerini doğrulayıp hesap/havuz kurulum durumunu güvenli ve salt okunur biçimde görünür kılmak; envanter sayımını gerçek istekte kullanılan hesap kanıtıyla karıştırmamak. Kabul işleri: HTTPS üzerinden kullanıcının ilk fiziksel cihazını bağlayıp PWA/push kabulünü yapmak; Claude hesabında kullanılabilir kapasite sağlandığında kurulu runner plan pilotunu ve yeni hata açıklamasının gerçek sağlayıcı eşliğini doğrulamak. Hesap/bütçe kurulumu ayrı kalır. Yerel `tasks/` altındaki test raporları ve özel pilot betikleri Git'e eklenmez; önceki tamamlanmış pilotları otomatik tekrarlamayın.

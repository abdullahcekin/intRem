...
Kurulu OmniRoute 3.8.50, `GET /api/providers?limit=1`, `GET /api/combos?limit=1` ve `GET /api/model-combo-mappings?limit=1` için yönetim erişimi ister. CLI erişim token'larında `read` kapsamı yeterlidir. Gateway yöneticisi resmi yerel CLI/API üzerinden bu kapsamda süreli bir token oluşturabilir. `INTREM_OMNIROUTE_TOKEN` değerini yalnız API servisine ait ayrı, 600 izinli bir env dosyasında tutun. `intrem-api.service.d/omniroute.conf` drop-in dosyasındaki `[Service]` bölümüne `EnvironmentFile=/home/USER/.config/intrem/gateway-read.env` ekleyin; `daemon-reload` ve yalnız API yeniden başlatması yapın. Ortak API/runner env dosyasına token koymayın. Token'ı sohbete, tarayıcıya, issue'ya veya Git'e koymayın; inference anahtarı bunun yerine geçmez, daha geniş yetki gerekmez. Süresi dolmadan aynı dar kapsamda yenileyin, eski token'ı iptal edin; süresi dolan bağlantı “Yetki gerekiyor” olarak görünür.

intRem yalnız bu üç GET isteğinde Bearer token gönderir, yönlendirme izlemez ve yanıttan yalnız toplam sayıları aktarır. Her okuma 5 saniye ve 64 KiB ile sınırlıdır; sonuçlar 30 saniye önbelleklenir. Token yoksa, geçersizse veya yetkisi yetersizse ilgili sayaç “Yetki gerekiyor”; ağ, şema veya servis hatasında “Bilinmiyor” görünür. `0` yalnız doğrulanmış boş kayıt demektir. Eşleme toplamı etkin ve devre dışı kayıtları birlikte sayar; sayılar hesapların çalıştığını, ayrı havuzların doğru kurulduğunu, gerçek isteğin hesabını/modelini veya bütçe güvencesini doğrulamaz. Ücretli fallback ayrı kabul tamamlanana kadar kapalıdır.
